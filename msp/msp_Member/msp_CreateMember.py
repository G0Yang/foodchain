...
class createMember:
    def createMember(self):
        ...
        _id = ""
        id = input('id : ')
        if(self.validCheckID(id)):
           return
        
        password = input('password : ')
        type = input("type(Producer:p/P, Distributor:d/D, Auctioneer:a/A, Ware-House Manager:w/W, Seller:s/S: ")
    ...
    def validCheckID(self, checkID):
        for member in collection.find({"ID": checkID}):
            print(member['ID'],"\nid 중복")
            return True
        return False
