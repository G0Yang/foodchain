...
# 상훈 202.31.146.58
# PORT 9009
-> 리눅스 상에서 경로참조에 문제가 있음.
-> 추후 테스트할 PC의 IP로 바꾸면 문제없음.
-> 서버 192.168.30.10과 192.168.30.20 사이에서 공인 202.31.146.57과 통신 확인함.
2. 리눅스 서버에서 foodchain을 구동하기위한 post작업들을 모아 쉘 스크립트 제작 완료
-> 192.168.30.20 에서 포맷 후 작업확인.

2018.08.26 패치노트
1. Foodchain.py 와 test.py 에서 쓰이는 소켓 서버 파일이 중복되어 분리함.
-> Foodchain.py 는 socket_Json.py
-> test.py 는 server_Json.py 를 사용.
2. metadata.py 의 데이터를 딕셔너리로 저장.
-> 나중에 암호화를 하여 데이터를 보낼 경우 같이 보낼 수 있음.
3. client.py 내용 지움.
-> 실제 GUI 환경에서 클라이언트가 해야 할 기능들을 추가할 계획
-> 로그인, 각 유저별 기능 제한, 로그아웃, 트랜잭션 파일별로 생성 등등...
4. msp 폴더에 mongoDB 기반의 맴버쉽관리 소스 구현
-> 간단한 x.509 인증서 발급과 비대칭키(개인키, 공개키) 생성 소스 구현

2018.08.28 패치노트
1. msp 기능 추가
-> msp에 등록된 맴버로서 ID와 PW를 입력하여 로그인을 하는 기능 추가.
-> tx 제안서를 배포할 때 이해관계자 리스트를 msp에서 받아오는 방식으로 변경.

2018.08.30 패치노트
1. 합의를 위한 소켓 통신 열어둠.
-> 소켓으로 찬성의사를 반환하도록 제작.
-> msp에서 요청받은 이해관계자 리스트 중 반대 의사가 없다면, 블록제작, 암호화, 블록전송 기능을 활성화함.
2. 각 노드는 자신의 역활에 맞게 msp에 요청을 하고, 역활에 맞는 트랜잭션을 블록에 추가할 수 있도록 제작.
